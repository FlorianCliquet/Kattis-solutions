...
struct avl_tree {
    struct node {
        ...
        node(const T& _item, node* _p = nullptr) 
            : item(_item), p(_p), l(nullptr), r(nullptr), size(1), height(0) {}
    };
    ...
    inline int64_t height(node* n) const { return n ? n->height : -1; }
    ...
    inline bool too_heavy(node* n) const { return n && abs(height(n->l) - height(n->r)) > 1LL; }
    ...
    node* nth(size_t n, node* cur = nullptr) const {
        if (!cur) cur = root;
        while (cur) {
    ...
    void clear() { delete_tree(root); root = nullptr; }
};
